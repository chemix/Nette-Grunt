...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('usemin-handler', 'Using HTML markup as the primary source of information', function() {
    ...
    /*
    console.log("----------------------------------------");
    var netteConfig = grunt.config('nette-replace') || false;
    if (netteConfig){
      // console.log(util.inspect(netteConfig));
    }
    console.log("----------------------------------------");
    */
    ...
    var concat = grunt.config('concat') || {},
      ...
      mincss = grunt.config('mincss') || { compress: { files: {} }};
    ...
    files.forEach(function(file) {
      ...
      Object.keys(blocks).forEach(function(dest) {
        var lines = blocks[dest].slice(1, -1),
          ...
          output = parts[1];
          // Nette hack
          output = output.replace('{$basePath}','www');
          output = output.replace('{$cdnUrl}','cdn');
        ...
        var assets = lines.map(function(tag) {
          ...
          // console.log('==========');
          if (asset!==undefined){
            asset = asset.replace('{$basePath}','www');
            asset = asset.replace('{$cdnUrl}','cdn');
          }
          // console.log('==========');
          // asset = asset.replace('{$cdnUrl}','cdn');
        ...
        if(type === 'css') {
          // Nette hack
          // add support for minify css
          mincss.compress.files[output] = output;
    ...
    grunt.log.subhead('Configuration is now:')
      .subhead('  css:')
      .writeln('  ' + util.inspect(css))
      .subhead('  concat:')
      .writeln('  ' + util.inspect(concat))
      .subhead('  min:')
      .writeln('  ' + util.inspect(min))
      .subhead('  rjs:')
      .writeln('  ' + util.inspect(rjs))
      .subhead('  mincss:')
      .writeln('  ' + util.inspect(mincss));
